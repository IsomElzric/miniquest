...
import arcade.gui # For UI elements like text input
...
CC_NAME_INPUT_WIDTH = CC_DESC_AREA_WIDTH - 40 # Width for the name input box
...
class CharacterCreationView(arcade.View):
    def __init__(self, previous_view: MenuView):
        super().__init__()
    ...
    def _setup_ui(self):
        ...
        name_input_y = SCREEN_HEIGHT - PLAYER_INFO_BANNER_HEIGHT - CC_DESC_AREA_Y_START_OFFSET - CC_NAME_INPUT_HEIGHT - 10
        self.name_input_box = arcade.gui.UIInputText(
            x=CC_DESC_AREA_X + CC_DESC_AREA_WIDTH / 2, # Centered in description area
            y=SCREEN_HEIGHT - PLAYER_INFO_BANNER_HEIGHT - CC_DESC_AREA_Y_START_OFFSET, # Top of description area
            width=CC_NAME_INPUT_WIDTH,
            height=CC_NAME_INPUT_HEIGHT,
            font_size=18,
            text="Adventurer", # Default text
            text_color=arcade.color.WHITE
        )
        self.manager.add(arcade.gui.UIAnchorWidget(child=self.name_input_box, anchor_x="center_x", anchor_y="top"))

        # Background selection buttons (left panel)
        start_y = SCREEN_HEIGHT - PLAYER_INFO_BANNER_HEIGHT - CC_DESC_AREA_Y_START_OFFSET
        for i, bg_name in enumerate(self.background_options):
            ...
            button.center_y = start_y - CC_NAME_INPUT_HEIGHT - 30 - (i * (CC_BACKGROUND_BUTTON_HEIGHT + CC_BACKGROUND_BUTTON_SPACING)) # Adjusted Y for name input
    ...
    def on_draw(self):
        ...
        if self.selected_background_key:
            ...
            desc_start_y_offset = CC_DESC_AREA_Y_START_OFFSET + CC_NAME_INPUT_HEIGHT + 20 # Extra 20px padding
            desc_y = SCREEN_HEIGHT - PLAYER_INFO_BANNER_HEIGHT - desc_start_y_offset
    ...
    def on_mouse_press(self, x: float, y: float, button: int, modifiers: int):
        ...
        if clicked:
            ...
            if action.startswith("select_bg_"):
                ...
                if self.selected_background_key:
                    character_name = self.name_input_box.text if self.name_input_box and self.name_input_box.text.strip() else "Adventurer"
